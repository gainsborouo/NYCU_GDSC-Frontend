...
本次實作將整合以下三大功能，模擬一個完整的前後端專案架構，並在 GitHub Codespaces 上部署：

1. 靜態檔案服務：使用 Nginx 提供 React App 的 HTML、CSS、JS。
2. API 反向代理：將 `/api` 開頭的 HTTP 請求轉發到後端 FastAPI。
3. WebSocket 反向代理：將 `/ws` 的 WebSocket 連線代理到後端 FastAPI。
...
3. WebSocket 反向代理
   - `/ws`：建立 WebSocket 連線，後端收到訊息後直接 echo 回傳。

### 環境準備

1.  開啟 Codespace:

    - 在 GitHub Repo 頁面點擊 `「Code」→「Open with Codespaces」→「New codespace」`。
    - 等待 Codespace 環境建立完成。
...
2.  建置前端並啟動後端服務
    ...
    nohup uvicorn backend.app.main:app --host 0.0.0.0 --port 8000 --reload > /workspace/backend.log 2>&1 &
    ```

    - 前端會被建置到 `frontend/dist`。
    - 後端使用 `uvicorn` 啟動 FastAPI Server，並將 log 輸出到 `/workspace/backend.log`。
...
3.  Nginx 設定  
    ...
    - 修改重點
      ...
      - location `/ws`：設定必要的 header，支援 WebSocket 連線升級。
